...
def create_telegraph():
    key = mdb.get_key("TELEGRAPH_KEY") or None
    if key:
    ...
    try:
        ...
        if "SHORT_NAME_TOO_LONG" in str(er):
            telegraph_client.create_account(short_name = "bot",  author_url = profile_url, author_name = full_name)
        else:
            log.exception(er)
            return
        return telegraph_client
    else:
        mdb.set_key("TELEGRAPH_KEY", telegraph_client.get_access_token())
        return telegraph_client
