...
@client.on(events.NewMessage(pattern=r"\/dr")) 
async def demote(m):
    ...
    if a:
        await m.reply("User has been demoted")
        await m.client.send_message(LOG_CHAT,  f"{pram} is demote from now by @{sender.username}")
