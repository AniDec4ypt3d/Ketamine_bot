...
async def _(m , cards , gate_db , user_db , lang, *args,**kwargs ):
    ...
    if spam_time < timee:
        ...
        x = await loop.run_in_executor(None,spreedly_def.get,r)
        token = await loop.run_in_executor(None,spreedly_def.first,r,cc ,mes , ano , cvv)
        antidb.set(m.sender_id, int(time.time()))
        id = await loop.run_in_executor(None,spreedly_def.second,r , token , x)
        msg = await msg.edit(lang['mid'].format(gate_name = gate_db['gate_name'],taken = int(time.time()) - start_time,),parse_mode= 'HTML')
        respo = await loop.run_in_executor(None,spreedly_def.third,r, id )
        r_msg , avs , cvvs ,r_logo , r_status= respo
        msg = await msg.edit(lang['semi_final'].format(gate_name = gate_db['gate_name'],taken = int(time.time()) - start_time,),parse_mode= 'HTML')
        try:
            await msg.edit(lang['sp_final'].format(
            gate_name = gate_db['gate_name'],lista = lista,respo=r_msg,status=r_status,logo=r_logo,cvvs=cvvs,avs=avs,vendor = vendor,
            type = type,
            bank_name = bank_name,
            country = country,
