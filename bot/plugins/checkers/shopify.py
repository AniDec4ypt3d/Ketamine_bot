...
async def _(m , cards , gate_db , user_db , lang, *args,**kwargs ):
    ...
    if spam_time < timee:
        ...
        variantId = await loop.run_in_executor(None,shopify.first,r)
        token = await loop.run_in_executor(None,shopify.second,r,variantId)
        url = await loop.run_in_executor(None,shopify.third,r)
        antidb.set(m.sender_id, int(time.time()))
        authenticity_token = await loop.run_in_executor(None,shopify.fourth,r ,url)
        msg = await msg.edit(lang['mid'].format(gate_name = gate_db['gate_name'],taken = int(time.time()) - start_time,),parse_mode= 'HTML')
        ship = await loop.run_in_executor(None,shopify.five,r, url ,authenticity_token)
        due = await loop.run_in_executor(None,shopify.six,r,url,authenticity_token,ship)
        west = await loop.run_in_executor(None,shopify.seven,r,cc,mes,ano,cvv)
        msg = await msg.edit(lang['semi_final'].format(gate_name = gate_db['gate_name'],taken = int(time.time()) - start_time,),parse_mode= 'HTML')
        eight = await loop.run_in_executor(None,shopify.eight,r,url,authenticity_token,west)
        final = await loop.run_in_executor(None,shopify.nine,r,url)
        r_msg ,r_logo , r_status= final
        try:
            ...
            return await m.client.send_message(LOG_CHAT, f"BadRequestError in Spreedly Gate")
